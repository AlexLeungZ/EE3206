...
public class TestOne56046680 {

    public static void main(String[] args) {

        // Load the CSV data file. Each row is converted to an instance of Record and
        // saved to a list.
        String filepath = TestOne56046680.class.getResource("number-of-internet-users-by-country.csv").getFile();
        ...
        for (Record record : records) {
            ...
            if (record.getYear() == year) {
                ...
                if (map.containsKey(country)) {
                    Integer curVal = map.get(country) + 1;
                    System.out.println(curVal);
                    map.put(country, curVal);
                } else {
                    map.put(country, 1);
                }
    ...
    static List<Record> worldUsersByYear(List<Record> records) {
        ...
        List<Record> worldRecords = new ArrayList<>();

        for (Record record : records) {
            for (Record worldRecord : worldRecords) {
                if (worldRecord.getYear() == record.getYear()) {
                    Long user = worldRecord.getUsers();
                    worldRecord = new Record(record.getYear(), user + record.getUsers());
                } else {
                    worldRecords.add(new Record(record.getYear(), record.getUsers()));
                }
            }
    ...
    static List<Entry<Integer, Double>> yearsOftop3Growth(List<Record> records, String code) {
        ...
        // Integer curList[] = new Integer[10000];
        ...
        // Double growthList[] = new Double[10000];

        // for (int i = 1; i < growthList.length; i++) {
        // growthList[i] = (curList[i].doubleValue() - curList[i - 1].doubleValue()) /
        // curList[i - 1].doubleValue();
        // }
